...
title: 'Machine learning fundamentals for social scientists: a practical approach'
...
abstract: "Statistics became widely employed and prevailed in the landscape of all social sciences from the 1950s onward. It was accompanied by a widely agreed-upon epistemological paradigm, which is mainly based on Karl Popper's formalizations from the 1950s. Nevertheless, calls for a paradigm shift in the social sciences have been growing louder in recent years. Due to the internet, a vast amount of new data sources have emerged and corresponding methods to analyze this new data are developed rapidly. These new methods are often commonly summarized under the term ‘machine learning’. This article aims to explore some of them and investigates if and how they can enrich the prevailing knowledge acquisition process in the social sciences. To do that, some practical examples will be given and advantages and limitations will be discussed along the way."
...
Research in the social sciences has been shifting towards a new era.\todo{RHH: check title}
...
Comparing logistic regressions and two popular [!ml] algorithms (Random Forest, Ridge Regression), we will explain how [!ml] works and, more importantly, how they are typically used by researchers outside the social sciences.
...
The quantification and numeric measurement of social phenomena, coupled with increasing computing power, can be seen as a “watershed moment” of empirical research in sociology [@mcfarland:SociologyEraBig.2016, 14]. 
...
In the wake of variable-driven data retrieval came the advent of regression models, factor analysis, and other statistics in sociology, although those methods were considered “esoteric expertise” [@abbottSystemProfessions1988, 147] until the 1960s.

Thus, the shift to quantitative research was fueled by fundamental changes in research content, techniques (surveys, regressions), and technology (computers).
...
It builds off inferential statistics and Karl Popper's formalizations [@popper:LogicScientificDiscovery.1968], i.e., creating falsifiable hypotheses, derived from (general) theories.
...
In addition, the efficient estimation and interpretation of regression coefficients depend on non-independent and identically distributed observations. 
...
Another important challenge for [!ts] relates to the ubiquity of *linear* models [@abbott:TranscendingGeneralLinear.1988]. 
...
One of the great promises of [!ml] is to offer ways to measure non-linear effects, as we will discuss below.
...
In contrast to the usage of inferential statistics, the application of [!ml] does not (yet) rest on established research in the social sciences. 
...
Researchers might use the output as input for further analysis [@heibergerFacetsSpecializationIts2021] or develop theoretical models [@hallIntroductionVarietiesCapitalism2001].
...
Topic models represent one of the most used [!nlp] tools in the social sciences [@mcfarland:DifferentiatingLanguageUsage.2013]. 
Also, the detection of communities resembles prominent ideas of social groups [@fortunato:CommunityDetectionGraphs.2010]. 
...
Almost all instances of [!ai] are [!sml], methodologically speaking, with the intent to predict an outcome with a given set of features.
...
There are two main answers to that important question, one epistemological, the other practical.
...
Those might also be based on information about what other customers might have bought. 
In any case, the wine shop will mostly be interested to develop models "that work", i.e., engineers might typically approach this task without any concern for theory or explain why somebody bought something. 
...
We will focus on illustrating the use of [!ml] for "classifications" instead of "regressions", because .… \todo{Argument fehlt.} 
...
To show the [!ml] workflow in its simplest form, binary classification, the resulting variable is binarized at the scale's mean value.
...
Therefore, Conservative individuals are expected to reject immigration.
...
Hence, we expect them to support immigration.
...
Country dummy variables are omitted for reasons of clarity.
...
It denotes the number of correctly classified data instances of the total number of data instances:

$$
Accuracy = \frac{TP + TN}{TP + FP + FN + TN}
$$
Accuracy is a good general measure as long as the data contains roughly equal amounts of [!tp] and [!tn] but is very susceptible to _class imbalance_, meaning that one class has a higher prevalence in the actual data than the other.
...
Both measurements are usually merged into one common number which is called the _F1-score_.
It is defined as the harmonic mean of both and therefore penalizes extremely low values in either:
$$
F1 = 2 * \frac{Precision * Recall}{Precision + Recall}
$$
To be comparable to the upcoming models, the logistic regression from above is rerun on the training data set and evaluated on the test set.
...
Common applications in [!ml] have to deal not only with a lot of rows in the data but also with a huge number of variables (or features in the [!ml] language).
...
After removing all features without any variance (111), as they cannot possibly be useful in any model, the resulting data set contains 759 remaining variables.

To handle a high amount of variables, or high dimensionality of the data, a common technique in [!ml] is to use penalized regressions.
...
Because it is desirable to use the test set only to measure the performance of the final model and therefore to avoid overfitting, a method named _K-Fold cross-validation_ can be applied.
It describes an approach in which the training data is split into $K$ (it is oftentimes set to 5 or 10)equally sized parts where $K-1$ parts are used for training and the remaining part is used for validation.
...
Since this can, depending on the number of hyperparameters, be an extremely computationally costly procedure, the parameter grid should be chosen with care.\todo{RHH: check if caret actually runs cv on gridsearch}
...
Or rather, the way we as social scientists can make use of the [!ml] perspective by … maybe finish by saying that [!uml] helps explore, [!sml] to predict\todo{ausformulieren}
